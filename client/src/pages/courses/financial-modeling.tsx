...
import { CheckCircle2, ArrowRight, Users, Award, GraduationCap, Clock, TrendingUp } from "lucide-react";
import { Link } from "wouter";
import { useLanguage } from "@/contexts/language-context";
import { FINANCIAL_MODELING_PAGE, financialModelingJsonLd } from "@/lib/i18n/financial-modeling-page";
...
export default function FinancialModelingPage() {
  ...
  useSEO({
    ...
    breadcrumb: [
      ...
      { name: "Financial Modeling", url: `${BASE_URL}/course/financial-modeling` },
    ],
    hreflang: [
      { lang: "en", url: `${BASE_URL}/course/financial-modeling` },
      { lang: "uz", url: `${BASE_URL}/course/financial-modeling?lang=uz` },
      { lang: "ru", url: `${BASE_URL}/course/financial-modeling?lang=ru` },
      { lang: "x-default", url: `${BASE_URL}/course/financial-modeling` },
    ],
    jsonLd: financialModelingJsonLd(lang, BASE_URL),
  });

  return (
    <Layout>
      <section className="relative overflow-hidden bg-gradient-to-br from-[#0a1a2a] via-slate-800 to-slate-900 pb-16 pt-5 sm:pb-20 lg:pt-24">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <nav className="mb-6 flex items-center gap-1.5 text-xs text-zinc-400">
            ...
            <span>/</span>
            <span className="font-semibold text-white">Financial Modeling</span>
          </nav>

          <div className="grid gap-10 lg:grid-cols-[1fr_400px] lg:gap-14 lg:items-start">
            <div>
              <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-teal-400/30 bg-teal-400/10 px-3 py-1.5 text-xs font-semibold text-teal-300">
                <TrendingUp className="h-3.5 w-3.5" /> {tx.badge}
              </div>
              ...
              <div className="mt-8">
                <a href="#cta">
                  <Button className="gap-2 rounded-full bg-teal-600 px-8 font-bold text-white hover:bg-teal-700">
            ...
            <div className="rounded-xl border border-white/10 bg-zinc-900 p-6 shadow-2xl">
              ...
              <LazyLeadForm source="course-financial-modeling" />
            </div>
          </div>
        </div>
      </section>
            ...
            {tx.learn.map((item, i) => (
              <div key={i} className="rounded-xl border border-white/10 bg-zinc-900 p-5">
                <div className="mb-2 flex items-center gap-3">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-teal-500/20 text-sm font-extrabold text-teal-300">{i + 1}</div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {tx.forWhom.map((item, i) => (
              <div key={i} className="flex items-start gap-4 rounded-xl border border-white/10 bg-zinc-900 p-5">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-teal-900/20 text-2xl border border-teal-500/20">
                  {item.emoji}
                </div>
                <div>
      ...
      <section className="py-16 sm:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {tx.why.map((item, i) => (
              <div key={i} className="flex items-start gap-3 rounded-xl border border-white/10 bg-zinc-900 p-5">
                <TrendingUp className="mt-0.5 h-5 w-5 shrink-0 text-teal-400" />
                <span className="text-sm font-medium text-zinc-200">{item}</span>
              </div>
            ))}
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {tx.format.map((item, i) => (
              <div key={i} className="flex items-center gap-3 rounded-xl border border-white/10 bg-zinc-900 p-5">
                <Clock className="h-5 w-5 shrink-0 text-teal-400" />
                <span className="text-sm font-medium text-zinc-200">{item}</span>
              </div>
            ))}
          </div>
      ...
      <section className="py-16 sm:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 rounded-xl border border-teal-500/30 bg-teal-900/20 p-6">
            <div className="flex items-center gap-3">
              <GraduationCap className="h-8 w-8 text-teal-400" />
              <div>
                <h3 className="text-base font-extrabold text-white">{tx.portfolioTitle}</h3>
                <p className="mt-1 text-sm text-zinc-400">{tx.portfolioDesc}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="cta" className="scroll-mt-20 bg-gradient-to-r from-teal-950 via-teal-800 to-slate-900 py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
            <div>
              <h2 className="text-2xl font-extrabold text-white sm:text-4xl">{tx.finalTitle}</h2>
              <p className="mt-4 text-teal-100 leading-relaxed">{tx.finalDesc}</p>
              <div className="mt-6 flex flex-wrap gap-4 text-sm text-teal-100">
            ...
            <div className="rounded-xl bg-white/10 p-6 backdrop-blur-sm">
              ...
              <DeferredLeadForm source="course-financial-modeling-cta" />
            </div>
          </div>
        </div>
      </section>
